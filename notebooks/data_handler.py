...
#
# adata = at.pp.add_metadata(adata, df_metadata, axis=0)
#
# file_path = "/Users/mschwoerer/Downloads/alphatools_test_data/AlphaDIA_1.9.2/second_pass_Lumosmodel/precursors.tsv"
...
#     pattern = r".*_(?P<replicate>Map\d+)_(?P<fraction>\d+K).*"
#
#     # Process each filename
#     for filename in file_names:
#         match = re.match(pattern, filename)
#         if match:
#             # Extract metadata and add the original filename
#             metadata = {
#                 'filename': filename,
...
#     metadata_df = pd.DataFrame(metadata_list).set_index("filename")
#     return metadata_df
#
# df_metadata=extract_metadata(adata.obs_names.to_numpy())
# adata = at.pp.add_metadata(adata, df_metadata, axis=0)
