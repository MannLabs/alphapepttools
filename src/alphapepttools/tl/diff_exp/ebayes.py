# Empirical Bayes moderated t-test for differential expression, with multiple contrasts,
# NaN handling and covariate support (eBayes moderation via inmoose's limma)
...
_METHOD_NAME = "limma_ebayes_inmoose"
...
def _standardize_contrast_frame(
    ...
    -------
    pd.DataFrame
        Results frame carrying the shared `tl_defaults.DIFF_EXP_COLS` columns in that order, followed
    ...
    # Lead with the shared column contract so this method stays aligned with the other diff_exp
    # methods, then append the method-specific columns (as alphaquant_wrapper does).
    return df[[*tl_defaults.DIFF_EXP_COLS, "stat"]]
...
def diff_exp_ebayes(
    ...
    Handles missing values without imputation, resolves several contrasts against a shared reference
    ...
    -------
    pd.DataFrame
        The standardized Limma eBayes results for every contrast, stacked into a single frame indexed by
        feature, carrying the shared `tl_defaults.DIFF_EXP_COLS` columns followed by this method's own
        `stat` column (the moderated t-statistic). Fold changes are reported as A - B (i.e.
        comparison[0] - comparison[1]), and the `condition_pair` column names each contrast "A_VS_B";
        select a single contrast by filtering on it.
    ...
    Adjust for a confounder that is unevenly distributed across the conditions, and require at least
    three observed values on each side before a fold change is reported:
    ...
    Compare several conditions against one shared reference in a single fit, then pick out one
    contrast. Passing ``"_ALL_"`` as the first element compares every other condition against the
    reference:
    ...
    alphapepttools.pp.filter_data_completeness : Upstream completeness filtering.
    ...
    ebayes_results = _ebayes_moderation(
    ...
    for contrast_idx, contrast_name in enumerate(contrast_names):
        ...
        t_values = ebayes_results["t"][contrast_idx].copy()
